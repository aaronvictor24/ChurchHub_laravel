...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-3xl font-bold text-gray-800">➕ Add New Event</h2>
    ...
    <div class="bg-white shadow rounded-xl p-6">
        <form action="{{ route('secretary.events.store') }}" method="POST">
            @csrf
            ...
            <div class="mt-6">
                ...
                <textarea name="description" rows="4"
                    class="mt-1 w-full px-3 py-2 border rounded-lg focus:ring focus:ring-green-300 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
