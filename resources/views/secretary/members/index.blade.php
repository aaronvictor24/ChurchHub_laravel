    ...
    <div class="flex items-start gap-6">
        ...
        <div class="flex-1">
            ...
            <div class="mb-8 no-print">
                ...
                <p class="text-gray-400 mt-1">Manage and view church members</p>
            </div>

            <!-- Search Bar -->
            <div class="mb-6 flex items-center gap-3 no-print">
                <form action="{{ route('secretary.members.index') }}" method="GET" class="flex-1 flex gap-3">
                    <input type="search" name="q" value="{{ request('q') }}" placeholder="Search members..."
            ...
            <div class="print-container" style="display:none;">
                ...
                @else
                    <div class="print-no-data">No members found</div>
                @endif
            </div>
            ...
            <div class="print-footer">
                <div class="print-footer-item">
                    Total Members: <strong>{{ $members->total() }}</strong>
                </div>
        ...
        <div class="w-72 no-print">
            ...
            <aside class="sidebar-filters bg-gray-800/60 rounded-lg p-6 sticky top-4 h-fit">
                ...
                <form action="{{ route('secretary.members.index') }}" method="GET" class="space-y-5">
                    <div>
                        <label class="block text-sm font-medium text-gray-200 mb-2">Gender</label>
                        <select name="gender"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-200 mb-2">Age Range</label>
                        <div class="flex gap-2">
                            <input type="number" name="age_min" value="{{ request('age_min') }}" placeholder="Min"
                                class="w-1/2 rounded-md bg-white/5 px-3 py-2 text-white text-sm placeholder-gray-500 outline-1 -outline-offset-1 outline-white/10 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500" />
                    ...
                    <div>
                        ...
                        <select name="sort_field"
                            class="w-full rounded-md bg-white/5 px-3 py-2 text-white text-sm outline-1 -outline-offset-1 outline-white/10 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500">
                            <option value="created_at" {{ request('sort_field') == 'created_at' ? 'selected' : '' }}>Latest
                                Added</option>
                            <option value="this_week" {{ request('sort_field') == 'this_week' ? 'selected' : '' }}>This
                                Week</option>
                            <option value="this_month" {{ request('sort_field') == 'this_month' ? 'selected' : '' }}>This
                                Month</option>
                            <option value="last_name" {{ request('sort_field') == 'last_name' ? 'selected' : '' }}>Last
                                Name</option>
                        </select>
                    </div>
                    <div>
