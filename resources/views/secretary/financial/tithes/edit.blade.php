...
@section('content')
    <div class="p-6 bg-gray-900 min-h-screen text-white">
        <h1 class="text-2xl font-bold mb-6">Edit Tithe</h1>
        <form action="{{ route('secretary.tithes.update', $tithe->tithe_id) }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <select name="member_id" class="w-full rounded bg-gray-700 text-white p-2">
                    ...
                    @foreach (App\Models\Member::all() as $member)
                        <option value="{{ $member->member_id }}" @if ($tithe->member_id == $member->member_id) selected @endif>
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="amount" class="w-full rounded bg-gray-700 text-white p-2"
                    value="{{ $tithe->amount }}" required>
            </div>
            <div class="mb-4">
                <label class="block mb-2 font-semibold">Date</label>
                <input type="date" name="date" class="w-full rounded bg-gray-700 text-white p-2"
                    value="{{ $tithe->date }}" required>
            </div>
            <div class="mb-4">
            ...
            <button type="submit"
                class="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-lg font-semibold">Update</button>
