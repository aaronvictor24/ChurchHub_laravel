    ...
    <div class="flex items-start gap-6">
        <div class="flex-1">
            <div class="print-header">
                <div class="print-header-top">
                    ...
                    <div class="print-header-text">
                        <h2 class="print-title">Offerings Report</h2>
                        @if (Auth::user() && Auth::user()->secretary && Auth::user()->secretary->church)
            ...
            <div class="mb-8 no-print">
                <h1 class="text-3xl font-bold text-white">Offerings</h1>
                <p class="text-gray-400 mt-1">View and manage church offerings</p>
            </div>

            <div class="mb-6 flex items-center gap-3 no-print">
                <form action="{{ route('secretary.financial.offerings') }}" method="GET" class="flex-1 flex gap-3">
                    <input type="search" name="q" value="{{ request('q') }}"
                        placeholder="Search offerings by mass, remarks or encoder..."
            ...
            <div class="print-container" style="display:none;">
                    ...
                    <table>
                        ...
                        <tbody>
                            @foreach ($offerings as $offering)
                                <tr>
                                    <td>{{ $offering->created_at->format('M d, Y') }}</td>
                                    <td>{{ $offering->mass->mass_title ?? 'N/A' }}</td>
                ...
                @else
                    <div class="print-no-data">No offerings found</div>
                @endif
            </div>

            <div class="print-footer">
                <div class="print-footer-item">
                    Total Records: <strong>{{ $offerings->total() }}</strong>
                </div>
        ...
        <div class="w-72 no-print">
            ...
            <aside class="sidebar-filters bg-gray-800/60 rounded-lg p-6 sticky top-4 h-fit">
                ...
                <form action="{{ route('secretary.financial.offerings') }}" method="GET" class="space-y-5">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="sort_by" class="w-full rounded-md bg-white/5 px-3 py-2 text-white text-sm">
                    ...
                    <div class="pt-5 border-t border-gray-600">
                        ...
                        <div class="space-y-2 text-sm text-gray-300">
                            <p>Total Records: <strong class="text-white">{{ $offerings->total() }}</strong></p>
                            <p>Showing: <strong class="text-white">{{ $offerings->count() }}</strong></p>
                        </div>
                    </div>
                </form>
            </aside>
