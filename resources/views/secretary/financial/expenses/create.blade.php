...
@section('content')
    <div class="p-6 bg-gray-900 min-h-screen text-white">
        <h1 class="text-2xl font-bold mb-6">Record Expense</h1>
        <form action="{{ route('secretary.expenses.store') }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <select name="purpose" class="w-full p-2 rounded bg-gray-700 text-white">
                    ...
                    @foreach ($purposes as $p)
                        <option value="{{ $p }}" {{ old('purpose') == $p ? 'selected' : '' }}>{{ $p }}
                        </option>
                    @endforeach
                </select>
                @error('purpose')
            ...
            <div class="mb-4">
                <label class="block mb-2 font-semibold">Released To</label>
                <input type="text" name="released_to" value="{{ old('released_to') }}"
                    class="w-full p-2 rounded bg-gray-700 text-white" placeholder="Name">
                @error('released_to')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="5" class="w-full p-2 rounded bg-gray-700 text-white" placeholder="Details">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
